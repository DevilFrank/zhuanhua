...
function allACtion(jskey, searchText = 'iphone', step = '', behaviorsId = '', countryCode = 'US') {
	...
	if (shouldCheckFixedBanner) {
		...
		if (largeBannerElements.length > 0) {
			...
			const { element, rect } = selectedBanner
			const fixedParent = element.parentElement
			const fixedParentRect = fixedParent ? fixedParent.getBoundingClientRect() : rect
			const fixedParentStyle = fixedParent ? window.getComputedStyle(fixedParent) : null
			const hasTopPosition = fixedParentStyle && fixedParentStyle.top !== 'auto'
			const hasBottomPosition = fixedParentStyle && fixedParentStyle.bottom !== 'auto'
			let bannerPosition = ''

			if (hasTopPosition && !hasBottomPosition) {
				bannerPosition = 'top'
			} else if (hasBottomPosition && !hasTopPosition) {
				bannerPosition = 'bottom'
			} else {
				const distanceToTop = Math.abs(fixedParentRect.top)
				const distanceToBottom = Math.abs(viewportHeight - fixedParentRect.bottom)
			...
			const trackData = {
				...
				foundElementCount: largeBannerElements.length,
				elementIds: largeBannerElements.map(item => item.element.id).filter(Boolean),
				selectedElementId,
			...
			JSBehavior.dotrack(26, JSON.stringify(trackData))
			JSBehavior.jsResult(jskey, position, jskey, false, false, behaviorsId)
			return
		}
	}
